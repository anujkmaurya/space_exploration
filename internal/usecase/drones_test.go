...
func TestUsecase_findLocation(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Test case 2: Positive case ",
			args: args{
				drone: &models.Drone{
					ID:       1,
					Name:     "drone1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := u.findLocation(tt.args.drone); got != tt.want {
				t.Errorf("Usecase.findLocation() = %v, want %v", got, tt.want)
			}
		})
	}
}
